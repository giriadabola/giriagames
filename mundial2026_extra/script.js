...
function applyVotingDeadlineUi() {
  refreshVotingDeadlineState();
  document.body.classList.toggle('voting-closed', isVotingClosed());
...
async function renderClosedPublicView() {
  ...
  container.innerHTML = `
    <section class="closed-view">
      <p class="eyebrow small">Votações encerradas</p>
      <h2>Prognósticos dos jogadores</h2>
      <p class="modal-muted">A partir de agora só podes consultar os prognósticos já gravados.</p>
      <div id="closedViewerBody" class="closed-viewer-body"><p class="modal-muted">A carregar prognósticos...</p></div>
    </section>
  `;

  if (!firestoreDb || !firebaseTools) {
    $('#closedViewerBody').innerHTML = '<p class="modal-muted">A preparar a consulta dos prognósticos...</p>';
    return;
  }

  try {
    await loadPublicPredictions();
    $('#closedViewerBody').innerHTML = renderPublicViewer('players');
  } catch (error) {
    console.error(error);
    $('#closedViewerBody').innerHTML = '<p class="modal-muted">Não foi possível carregar os prognósticos neste momento.</p>';
  }
}
