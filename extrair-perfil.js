...
import { buildProfile, computeFullPlayerRanking, scorePrediction } from './extrair-perfil-stats.js?v=20260702perfil9';
import { escapeHtml, renderProfile, renderVsProfile } from './extrair-perfil-render.js?v=20260702perfil8';
...
function updatePlayerDropdowns() {
  ...
  const afterMatchActive = state.activeFilters.includes('afterMatch');
  const leaderboardActive = state.activeFilters.includes('leaderboard');
  const showMatches = predictionActive || afterMatchActive || leaderboardActive;
...
function updatePredictionsPanel() {
  ...
  panelContent.innerHTML = buildPredictionsPanelHtml(player);
  return;
  ...
  const predictionsHtml = (player.matches || []).map(prediction => {
    ...
    if (hasOfficial) {
      const score = scorePrediction(prediction, official, state.data.scoringRules || {});
      points = score.points;
...
function buildPredictionsPanelHtml(player) {
  return (state.data.matches || []).map((match) => {
    const matchId = String(match.id);
    const pred = (player.matches || []).find((row) => String(row.id) === matchId);
    const override = getSection2DocForPlayer(player, match.id);
    ...
    let pointsClass = 'pending';
    if (hasOfficial) {
      const score = scorePrediction(displayPred, official, state.data.scoringRules || {});
      points = score.points;
      if (score.exact) pointsClass = 'exact';
      else if (points > 0) pointsClass = 'outcome';
      else pointsClass = 'miss';
    }
    ...
    let predictionText = `${displayPred.homeGoals} - ${displayPred.awayGoals}`;
    if (override && override.mode === 'changed' && override.winnerTeam) {
      let methodSuffix = '';
      if (override.method === 'et') methodSuffix = ' após prolongamento';
      else if (override.method === 'pens') methodSuffix = ' após penáltis';
      predictionText += ` <span class="muted" style="font-size:0.75rem;">(vence ${escapeHtml(override.winnerTeam)}${methodSuffix})</span>`;
    } else if (override && override.mode === 'replicate') {
      predictionText += ` <span class="muted" style="font-size:0.75rem;">(Mantido)</span>`;
    }
    ...
    return `
      <div class="prediction-item ${pointsClass}">
        ...
        <div class="prediction-scores">
          <div class="score-box pred">
            <span class="label">Prog:</span>
            <span class="val">${predictionText}</span>
          </div>
          ${hasOfficial ? `
            <div class="score-box real">
              <span class="label">Res:</span>
    ...
    `;
  }).join('') || '<div class="empty">Sem prognósticos.</div>';
}
