...
export const FILTERS = {
  ...
  leaderboard: { title: 'Tabela Classificativa', text: 'Tabela classificativa geral do torneio. Desempate por pontos, acertados, GM, menos GF e nome.' },
  ...
  afterMatch: { title: 'After Match', text: 'Análises de desfecho do jogo selecionado.' }
};
...
function getActivePrediction(player, matchId, data) {
  const match = data.matches.find(m => String(m.id) === String(matchId));
  const isKO = match && match.stage !== 'groups';
  if (isKO) {
    const reform = getKnockoutPrediction(player, matchId, data);
    if (reform) return reform;
    return null;
  }
  return (player.matches || []).find(p => String(p.id) === String(matchId));
}

function findPredictionForMatch(player, match, data, resolvedHome = null, resolvedAway = null) {
  if (!player || !match) return null;

  const matchId = String(match.id);
  const exactPred = (player.matches || []).find(p => String(p.id) === matchId) || null;
  const stage = match.stage || 'groups';
  const homeTeam = resolvedHome || match.homeTeam || match.home || '';
  const awayTeam = resolvedAway || match.awayTeam || match.away || '';

  if (stage === 'groups') {
    return exactPred
      ? {
          ...exactPred,
          homeTeam: exactPred.homeTeam || exactPred.home || homeTeam,
          awayTeam: exactPred.awayTeam || exactPred.away || awayTeam,
          stage: exactPred.stage || stage,
          id: exactPred.id || match.id
        }
      : null;
  }

  let initialPred = exactPred;
  if (!initialPred && homeTeam && awayTeam) {
    initialPred = (player.matches || []).find(p => p.stage === stage && sameMatchupAnySide(p, { homeTeam, awayTeam })) || null;
  }

  const reform = getKnockoutPrediction(player, matchId, data);
  if (reform) {
    const basePred = reform.initialPrediction || initialPred || exactPred || null;
    return {
      ...(basePred || {}),
      ...reform,
      homeTeam: reform.homeTeam || basePred?.homeTeam || basePred?.home || homeTeam,
      awayTeam: reform.awayTeam || basePred?.awayTeam || basePred?.away || awayTeam,
      stage: reform.stage || basePred?.stage || stage,
      id: reform.matchId || basePred?.id || match.id
    };
  }
...
function buildRows(data, players) {
  return players.flatMap((player) => {
    return (data.matches || []).map((match) => {
      ...
      if (!official || !hasScore(official)) return null;

      const activePred = findPredictionForMatch(player, match, data, official.homeTeam, official.awayTeam);
      if (!activePred || !hasScore(activePred)) return null;

      const sofa = data.sofaByMatchId[matchId];
      const events = goalEvents(match, sofa);
      const transitions = buildTransitions(player, activePred, match, events, data.scoringRules);
      return { player, prediction: activePred, match, official, sofa, events, transitions, scoringRules: data.scoringRules };
    }).filter(Boolean);
  });
}
...
function transitionSummary(items, tone) {
  const sorted = [...items].sort((a, b) => 
    ...
    Number(b.match.id) - Number(a.match.id) || 
    b.event.minuteValue - a.event.minuteValue
  );
...
export function scorePrediction(pred, official, scoringRules) {
  ...
  const officialWinner = officialWinnerTeam(official);
  const winnerHit = teamKey(predictedWinnerTeam(pred)) === teamKey(officialWinner);
  const exact = exactKnockoutPrediction(pred, official, stage, sidesOk);
  const isReformed = !!pred.isReformed;
  
  let points = 0;
  if (stage === 'groups') {
    points = exact ? (scoringRules.groupExact ?? 3) : (sidesOk && predicted === actual ? (scoringRules.groupOutcome ?? 1) : 0);
  } else if (isReformed) {
    if (exact) {
      if (stage === 'round32') points = scoringRules.reformExact32 ?? 4;
      else if (stage === 'round16') points = scoringRules.reformExact16 ?? 4;
      else if (stage === 'quarterfinals') points = scoringRules.reformExact8 ?? 4;
      else if (stage === 'semifinals') points = scoringRules.reformExact4 ?? 5;
      else if (stage === 'third_place') points = scoringRules.reformExact3rd ?? 5;
      else if (stage === 'final') points = scoringRules.finalReformExact ?? 3;
      else points = scoringRules.knockoutReformExact ?? 2;
    } else if (winnerHit) {
      if (stage === 'round32') points = scoringRules.reformWinner32 ?? 1;
      else if (stage === 'round16') points = scoringRules.reformWinner16 ?? 1;
      else if (stage === 'quarterfinals') points = scoringRules.reformWinner8 ?? 1;
      else if (stage === 'semifinals') points = scoringRules.reformWinner4 ?? 2;
      else if (stage === 'third_place') points = scoringRules.reformWinner3rd ?? 2;
      else if (stage === 'final') points = scoringRules.reformWinnerFinal ?? 2;
      else points = scoringRules.knockoutReformWinner ?? 1;
    }
  } else {
    if (exact) {
      ...
      else if (stage === 'final') points = scoringRules.initialExactFinal ?? 11;
      else points = stage === 'final' ? (scoringRules.finalInitialExact ?? 8) : (scoringRules.knockoutInitialExact ?? 6);
    } else if (winnerHit) {
      ...
      else points = stage === 'final' ? (scoringRules.finalInitialWinner ?? 4) : (scoringRules.knockoutInitialWinner ?? 2);
    }
  }
...
function predictedWinnerTeam(pred) {
  if (!pred) return '';
  if (pred.winnerTeam && pred.winnerTeam !== 'Empate') return pred.winnerTeam;
  const ph = Number(pred.homeGoals);
  const pa = Number(pred.awayGoals);
  if (ph > pa) return pred.homeTeam || pred.home || '';
  if (pa > ph) return pred.awayTeam || pred.away || '';
  return 'Empate';
}
...
function officialWinnerTeam(official) {
  ...
  const oh = Number(official.homeGoals);
  const oa = Number(official.awayGoals);
  if (oh > oa) return official.homeTeam || official.home || '';
  if (oa > oh) return official.awayTeam || official.away || '';
  return 'Empate';
}
...
function collectExactHitsByMatch(data) {
  ...
  data.players.forEach(player => {
    (data.matches || []).forEach(match => {
      ...
      const prediction = findPredictionForMatch(player, match, data, official.homeTeam, official.awayTeam);
      if (!prediction || !hasScore(prediction)) return;

      const score = scorePrediction(prediction, official, data.scoringRules || {});
      if (!score.exact) return;
  ...
  return exactHitsByMatch;
}

function aloneInFameSummary(data, selectedPlayers) {
  const exactHitsByMatch = collectExactHitsByMatch(data);
...
function compareGgamesRankingRows(a, b, pointsKey = 'points') {
  return (Number(b?.[pointsKey] || 0) - Number(a?.[pointsKey] || 0)) ||
    (Number(b?.correctPredictions || 0) - Number(a?.correctPredictions || 0)) ||
    (Number(b?.goalsHit || 0) - Number(a?.goalsHit || 0)) ||
    (Number(a?.goalsMissed || 0) - Number(b?.goalsMissed || 0)) ||
...
function calculateBattleResult(battle, data) {
  ...
  const match = (data.matches || []).find(m => String(m.id) === matchId) || official || null;
  ...
  const predA = match ? findPredictionForMatch(playerA, match, data, official.homeTeam, official.awayTeam) : null;
  const predB = match ? findPredictionForMatch(playerB, match, data, official.homeTeam, official.awayTeam) : null;
...
function findInitialPredictionForResolvedMatchup(player, match, home = null, away = null) {
  if (!player || !match) return null;
  const matches = player.matches || [];
  const matchId = Number(match.id || match.matchId);
  const exactPred = matches.find(row => Number(row.id) === matchId) || null;

  if (match.stage && match.stage !== 'groups') {
    const resolvedOfficial = { homeTeam: home || match.homeTeam || match.home, awayTeam: away || match.awayTeam || match.away };
    if (exactPred && sameMatchupAnySide(exactPred, resolvedOfficial)) {
      return exactPred;
    }
    const matchupPred = matches.find(p => p.stage === match.stage && sameMatchupAnySide(p, resolvedOfficial)) || null;
...
function calculateStepMatchupPointsOptimized(player, resolvedMatchups, data) {
  ...
  resolvedMatchups.forEach(m => {
    const matchupPred = findInitialPredictionForResolvedMatchup(player, m, m.home, m.away);
...
function getResolvedRound32MatchupsAtStep(stepMatches, data, includePp) {
  if (!includePp) return [];
  ...
  return (data.matches || []).reduce((list, match) => {
    if (match.stage !== 'round32') return list;
    ...
    if (!isTeamUnresolved(home) && !isTeamUnresolved(away)) {
      list.push({ id: match.id, stage: match.stage, home, away });
    }
    return list;
  }, []);
}

function computeRankingsAtStepData(data, playedMatches, stepIdx, options = {}) {
  ...
  const resolvedMatchups = getResolvedRound32MatchupsAtStep(stepMatches, data, includePp);
  ...
  const playersPoints = data.players.map(player => {
    ...
    stepMatches.forEach(item => {
      const pred = findPredictionForMatch(player, item.match, data, item.official.homeTeam, item.official.awayTeam);
      if (!pred || !hasScore(pred)) return;

      const score = scorePrediction(pred, item.official, data.scoringRules || {});
      accumulated += score.points;

      if (score.points > 0) correctPredictions += 1;
      else failedPredictions += 1;
    ...
    if (includeBw) {
      (data.battleDocs || []).forEach(battle => {
        if (!stepMatchIds.has(String(battle.matchId))) return;
    ...
    const battleBonusPoints = Math.floor(battleWins / 2);
  ...
  return [...playersPoints]
    .sort((a, b) => compareGgamesRankingRows(a, b, 'totalPoints'))
...
function starsPathSummary(data, selectedPlayers, options = {}) {
  ...
  playedMatches.forEach((item, stepIdx) => {
    const stepSorted = computeRankingsAtStepData(data, playedMatches, stepIdx, { includeBw, includePp });

    stepSorted.forEach((pInfo) => {
      const actualPlayer = playersPoints.find(p => p.id === pInfo.id);
      actualPlayer.history.push({
        matchId: item.match.id,
        accumulated: pInfo.totalPoints,
        position: pInfo.rank
      });
    });
  });
...
export function sequenceSummary(data, selectedPlayers) {
  ...
  playedMatches.forEach(({ match, official }) => {
    selectedPlayers.forEach(player => {
      const pred = findPredictionForMatch(player, match, data, official.homeTeam, official.awayTeam);
      if (!pred || !hasScore(pred)) return;
  ...
  if (key === 'lost' || key === 'dramaticLost' || key === 'gains' || key === 'lateGains') {
    ...
    for (const [matchId, playerIds] of exactHitsByMatch.entries()) {
      if (playerIds.length === 1) {
        const p = playerScores.get(playerIds[0].player.id);
        if (p) p.score += 1;
      }
    }
...
function getKnockoutPrediction(player, matchId, data) {
  ...
  const isReformed = reformDoc.mode === 'changed' || reformDoc.status === 'section2' || reformDoc.type === 'knockoutRevision';
  ...
  return {
    ...
    method,
    mode: reformDoc.mode,
    initialPrediction: reformDoc.initialPrediction || null,
    homeTeam: reformDoc.homeTeam || homeTeam,
    awayTeam: reformDoc.awayTeam || awayTeam,
    isReformed,
    stage: matchStage
  };
}
...
export function predictionSummary(data, selectedMatchId, selectedPlayers) {
  ...
  const predictions = data.players
    .filter(player => {
      if (isKO) {
        return !!getKnockoutPrediction(player, matchId, data);
      }
      return !!findPredictionForMatch(player, match, data, official?.homeTeam, official?.awayTeam);
    })
    .map(player => {
      const pred = findPredictionForMatch(player, match, data, official?.homeTeam, official?.awayTeam);
      let score = null;
...
export function leaderboardSummary(data, selectedMatchId, options = {}) {
  ...
  const computeRankingsAtStep = (stepIdx) => computeRankingsAtStepData(data, playedMatches, stepIdx, { includeBw, includePp });
